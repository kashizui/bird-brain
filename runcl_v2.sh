...
# Usage:
#
#   ./runcl.sh [-d data] [-s src] -- [--gpu] [--resume UUID EPOCH] [--host NODENAME] [ARGS [ARGS ...]]
#
# --gpu                 Use GPU
...
command="cl run --tail -n run-train $OPTIONS --request-docker-image $IMAGE_NAME $DEPENDENCIES src:$SRC data:$DATA --- python src/run.py $RUNARGS ${@}"
echo $command
